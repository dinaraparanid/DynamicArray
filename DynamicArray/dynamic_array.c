...
void* __get_from_dyn_array__(dynamic_array* arr, const size_t ind)
{
...
void* __pop_from_dyn_array__(dynamic_array* arr) {
	return (int8_t*)arr->arr_data->__data__ + --arr->arr_data->__size__ * arr->arr_data->__size_of_elem__;
}
...
void __insert_cloned_to_dyn_array__(dynamic_array* arr, const size_t ind, const void* const elem)
{
	const size_t size = arr->arr_data->__size__;
...
void* __bin_search_rule_dyn_array__(const dynamic_array arr, const void* const key, int (*cmp)(const void*, const void*)) {
	return bsearch(key, arr.arr_data->__data__, arr.arr_data->__size__, arr.arr_data->__size_of_elem__, cmp);
}

size_t __find_first_dyn_array__(const dynamic_array arr, const void* const key)
{
...
size_t __find_last_dyn_array__(const dynamic_array arr, const void* const key)
{
	int8_t* data = arr.arr_data->__data__;
...
dynamic_array* __filter_dyn_array__(const dynamic_array arr, int (*f)(const void*))
{
	...
	for (size_t i = 0; i < arr.arr_data->__size__; i++)
	{
		...
		if (f(elem)) filtered->push_cloned(filtered, elem);
	}

	return filtered;
}

void* __fold_dyn_array__(const dynamic_array arr, const void* const start, const size_t size_of_start, void*(*act)(const void*, const void*))
{
...
dynamic_array* __scan_dyn_array__(const dynamic_array arr, const void* const start, const size_t size_of_start, void* (*act)(const void*, const void*))
{
...
dynamic_array* __sub_dyn_array__(const dynamic_array arr, const size_t from, const size_t to)
{
	...
	const size_t size_of_elem = arr.arr_data->__size_of_elem__;
	const size_t total_size = to - from + 1;
	const size_t total_capacity = __calc_capacity__(total_size);
	...
	memmove(data->__data__, (int8_t*)arr.arr_data->__data__ + from * size_of_elem, total_size * size_of_elem);

	dynamic_array* sub = new_dyn_array(arr.arr_data->__size_of_elem__);
	sub->arr_data = data;
...
void* __first_dyn_array__(dynamic_array* arr) { return arr->get(arr, 0); }
void* __last_dyn_array__(dynamic_array* arr) { return arr->get(arr, arr->arr_data->__size__ - 1); }

void __for_each_dyn_array__(dynamic_array* arr, void(*f)(void*))
{
	const size_t size_of_elem = arr->arr_data->__size_of_elem__;
	const size_t size = arr->arr_data->__size__;
	int8_t* data = arr->arr_data->__data__;
	...
	for (size_t i = 0; i < size; i++)
		f(data + i * size_of_elem);
}

dynamic_array* __on_each_dyn_array__(dynamic_array* arr, void(*f)(void*))
{
	const size_t size_of_elem = arr->arr_data->__size_of_elem__;
	const size_t size = arr->arr_data->__size__;
	int8_t* data = arr->arr_data->__data__;
...
size_t __find_first_by_dyn_array__(const dynamic_array arr, int (*f)(const void*))
{
	int8_t* data = arr.arr_data->__data__;

	for (size_t i = 0; i < arr.arr_data->__size__; i++)
		if (f(data + i * arr.arr_data->__size_of_elem__))
			return i;

	return arr.arr_data->__size__;
}

size_t __find_last_by_dyn_array__(const dynamic_array arr, int (*f)(const void*))
{
	int8_t* data = arr.arr_data->__data__;

	for (size_t i = arr.arr_data->__size__; i > 0; i--)
		if (f(data + (i - 1) * arr.arr_data->__size_of_elem__))
			return i - 1;

	return arr.arr_data->__size__;
}

int __contains_dyn_array__(const dynamic_array arr, const void* const key) { return arr.find_first(arr, key) != arr.arr_data->__size__; }
int __empty_dyn_array__(const dynamic_array arr) { return arr.arr_data->__size__ == 0; }
int __not_empty_dyn_array__(const dynamic_array arr) { return !arr.empty(arr); }

void __clear_dyn_array__(dynamic_array* arr)
{
	arr->arr_data->__capacity__ = 1;
	arr->arr_data->__size__ = 0;
	arr->arr_data->__data__ = realloc(arr->arr_data->__data__, 1 * arr->arr_data->__size_of_elem__);
}

void __free_dyn_array__(dynamic_array* arr)
{
	arr->arr_data->__capacity__ = 0;
	arr->arr_data->__size__ = 0;
	free(arr->arr_data->__data__);
	arr->arr_data->__data__ = NULL;
}
...
void __set_methods_dyn_array__(dynamic_array* arr)
{
	...
	arr->insert_cloned = __insert_cloned_to_dyn_array__;
	...
	arr->as_ptr = __get_data_dyn_array__;
	arr->first = __first_dyn_array__;
	arr->last = __last_dyn_array__;
	arr->for_each = __for_each_dyn_array__;
	arr->on_each = __on_each_dyn_array__;
	arr->contains = __contains_dyn_array__;
	arr->empty = __empty_dyn_array__;
	arr->not_empty = __not_empty_dyn_array__;
	arr->find_first_by = __find_first_by_dyn_array__;
	arr->find_last_by = __find_last_by_dyn_array__;
	arr->clear = __clear_dyn_array__;
	arr->free = __free_dyn_array__;
}
